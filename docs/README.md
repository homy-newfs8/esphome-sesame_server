...
上記の例では`deny`が指定された2つのデバイスからの接続があった場合は即時に切断し、他のデバイスから接続は許容します。
既知のアドレスに`allow`を指定し、最後に`deny`を指定することで登録したアドレスからの接続のみ許可することも可能ですが、スマホはBLEアドレスを定期的に変更してアクセスしてくるため、
スマホからのアクセスがくる毎にログにエラーが記録されることになります(本コンポーネントはスマホへの登録が完了してしまえば、以降はスマホで操作することはないので実用上の問題は生じないと思われます)。
